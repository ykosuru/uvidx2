...
Examples:
  ...
  python build_index.py --pdf-dir ./docs --tal-dir ./code --output ./my_index \\
    ...
    # Index PDFs
    if args.pdf_dir:
        pdf_stats = index_pdf_directory(pipeline, args.pdf_dir, recursive, manifest)
        total_stats["pdf"] = pdf_stats
    
    # Index TAL code
    if args.tal_dir:
        tal_stats = index_tal_directory(pipeline, args.tal_dir, recursive, manifest)
        total_stats["tal"] = tal_stats
    
    # Index generic code (C, C++, Java, Python, etc.)
    if args.code_dir:
        code_stats = index_code_directory(pipeline, args.code_dir, recursive, manifest)
