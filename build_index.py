...
def index_single_file(pipeline: IndexingPipeline, 
    ...
    try:
        ...
        chunks = pipeline.index_content(content, str(path), source_type, domain=domain)
...
Examples:
  ...
  # Add a single file
  python build_index.py --add-file ./new_doc.pdf --output ./my_index
  ...
  python build_index.py --pdf-dir ./docs --tal-dir ./code --output ./my_index \\
    ...
    # Domain options
    parser.add_argument("--domain", "-D", type=str, default="default",
                        help="Domain tag for all indexed files (default: 'default')")
    parser.add_argument("--domain-map", type=str, default=None,
                        help="JSON file mapping domains to directories")
    parser.add_argument("--infer-domains", action="store_true",
                        help="Infer domain from parent directory name")
    ...
    # Validate inputs
    if not args.pdf_dir and not args.tal_dir and not args.code_dir and not args.add_file:
        print("Error: At least one of --pdf-dir, --tal-dir, --code-dir, or --add-file is required")
        sys.exit(1)
    ...
    # Determine domain(s) for indexing
    domain = args.domain
    
    # Handle domain mapping file
    domain_map = None
    if args.domain_map:
        try:
            with open(args.domain_map, 'r') as f:
                domain_map = json.load(f)
            print(f"Loaded domain map from: {args.domain_map}")
            print(f"Domains: {list(domain_map.keys())}")
        except Exception as e:
            print(f"Warning: Failed to load domain map: {e}")
    
    # Index individual files (--add-file)
    if args.add_file:
        print(f"\nAdding {len(args.add_file)} file(s) to index...")
        for file_path in args.add_file:
            # Infer domain from path if requested
            file_domain = domain
            if args.infer_domains:
                file_domain = os.path.basename(os.path.dirname(file_path)) or "default"
            file_stats = index_single_file(pipeline, file_path, manifest, domain=file_domain)
    ...
    # Index using domain map if provided
    if domain_map:
        for dom_name, dom_config in domain_map.items():
            directories = dom_config.get('directories', [])
            for directory in directories:
                if os.path.isdir(directory):
                    print(f"\nIndexing domain '{dom_name}' from: {directory}")
                    # Determine type from directory content
                    stats = index_tal_directory(pipeline, directory, recursive, manifest, domain=dom_name)
                    # Merge stats
                    for key in ['files_processed', 'chunks_created', 'files_failed', 'skipped']:
                        total_stats['tal'][key] = total_stats['tal'].get(key, 0) + stats.get(key, 0)
    else:
        # Index PDFs
        if args.pdf_dir:
            pdf_domain = domain
            if args.infer_domains:
                pdf_domain = os.path.basename(os.path.normpath(args.pdf_dir)) or "documents"
            pdf_stats = index_pdf_directory(pipeline, args.pdf_dir, recursive, manifest, domain=pdf_domain)
        ...
        if args.tal_dir:
            tal_domain = domain
            if args.infer_domains:
                tal_domain = os.path.basename(os.path.normpath(args.tal_dir)) or "code"
            tal_stats = index_tal_directory(pipeline, args.tal_dir, recursive, manifest, domain=tal_domain)
        ...
        if args.code_dir:
            code_domain = domain
            if args.infer_domains:
                code_domain = os.path.basename(os.path.normpath(args.code_dir)) or "code"
            code_stats = index_code_directory(pipeline, args.code_dir, recursive, manifest, domain=code_domain)
