...
================================================================================
    ...
    --min-score       Minimum score for LLM analysis (default: 0.0)
...
def search_once(pipeline: IndexingPipeline, 
    ...
    results = pipeline.search(search_query, top_k=fetch_k, source_types=source_types, domains=domains)
...
def search_decomposed(pipeline: IndexingPipeline,
    ...
    if len(sub_queries) == 1:
        return search_once(pipeline, query, top_k, source_type, verbose,
                          knowledge_graph, expand_query, tfidf_boost, show_related, domains)
...
Interactive commands for knowledge graph:
  ...
  :graph            Show knowledge graph statistics
    ...
    parser.add_argument("--domain", "-D", type=str, default=None,
                        help="Filter by domain(s), comma-separated (e.g., 'payments,compliance')")
    parser.add_argument("--list-domains", action="store_true",
                        help="List available domains in index and exit")
    ...
    parser.add_argument("--min-score", type=float, default=0.0,
                        help="Minimum score for LLM analysis (default: 0.0)")
    ...
    if args.list_domains:
        ...
        if domains:
            ...
            print("\nNo domain information available (all chunks in 'default' domain)")
        return
    
    # Parse domain filter
    domains_filter = None
    if args.domain:
        domains_filter = [d.strip() for d in args.domain.split(',')]
        print(f"Domain filter: {', '.join(domains_filter)}")
